...
class Plotter:
    ...
    def plot_scale_size(self, d: int, rows: List[List[float | int | str]],range_pct:float):
        ...
        for ax, (name, idx, ylabel, bp_complexity, br_complexity) in zip(axes.flat, ops):
            # Plot empirical data
            ax.plot(sizes_bp, [r[idx] for r in bp], "o-", label="B+ Tree (empirical)", linewidth=2, color="steelblue")
            ax.plot(
                sizes_br, [r[idx] for r in br], "s--", label="Brute Force (empirical)", linewidth=2, color="salmon"
            )
            
            sizes_theory = np.logspace(np.log10(min(sizes_bp + sizes_br)), np.log10(max(sizes_bp + sizes_br)), 50)  # type:ignore
            ...
            if name == "insert_total_s":
                ...
                bp_theory = np.log2(sizes_theory) + sizes_theory * range_pct  # type:ignore
            ...
            ax.plot(sizes_theory, bp_theory, ":", linewidth=2.5, alpha=0.8, color="steelblue", label=f"B+ Tree Theory ({bp_complexity})")
            ax.plot(sizes_theory, br_theory, ":", linewidth=2.5, alpha=0.8, color="salmon", label=f"Brute Theory ({br_complexity})")
